...
function migrateOpenAIDefaultBaseUrl(
  baseUrl: string,
  provider: ApiProvider,
): string {
  if (provider !== "openai") return baseUrl;
  return baseUrl.trim().replace(/\/+$/, "") === LEGACY_DEFAULT_BASE_URL
    ? DEFAULT_BASE_URL
    : baseUrl;
}
  ...
  const entries = Object.entries(input)
    .map(
      ([provider, draft]) =>
        [
          provider,
          normalizeProviderDraft(draft, provider, customProviderIds),
        ] as const,
    )
    .filter(
...
export function normalizeApiProfile(
  ...
  const baseUrl = migrateOpenAIDefaultBaseUrl(rawBaseUrl, provider);
  ...
  return {
    ...
    id:
      typeof record.id === "string" && record.id.trim()
        ...
        : defaults.id,
    name:
      record.id === DEFAULT_OPENAI_PROFILE_ID && record.name === "默认"
...
export function normalizeSettings(
  ...
  const legacyProfile = createDefaultOpenAIProfile({
    baseUrl: migrateOpenAIDefaultBaseUrl(
...
export function getCustomProviderDefinition(
  ...
  return (
    normalized.customProviders.find((item) => item.id === provider) ?? null
  );
}
  ...
  if (Array.isArray(record.customProviders)) {
    ...
    const profiles = Array.isArray(record.profiles)
      ? record.profiles
          ...
          .map((item) =>
            normalizeApiProfile(item, undefined, customProviderIds),
          )
...
function hasOnlyDefaultProfiles(settings: AppSettings): boolean {
  return (
    ...
    (isDefaultOpenAIProfile(settings.profiles[0]) ||
      isLegacyDefaultOpenAIProfile(settings.profiles[0]))
  );
}
...
function hasEquivalentApiProfile(
  existingProfiles: ApiProfile[],
  importedProfile: ApiProfile,
): boolean {
  const dedupKey = getApiProfileDedupKey(importedProfile);
  if (
    existingProfiles.some(
      (profile) => getApiProfileDedupKey(profile) === dedupKey,
    )
  )
    return true;
  ...
  return existingProfiles.some(
    (profile) => getApiProfileConnectionKey(profile) === connectionKey,
  );
}
...
function mergeImportedCustomProviders(
  ...
  const existingKeys = new Map(
    providers.map(
      (provider) => [getCustomProviderDedupKey(provider), provider.id] as const,
    ),
  );
  ...
  const importedProfiles = imported.profiles
    .map((profile) =>
      providerIdMap.has(profile.provider)
        ? {
            ...profile,
            provider: providerIdMap.get(profile.provider) ?? profile.provider,
          }
        : profile,
    )
    .filter(
      (profile) =>
        !existingKeys.has(getApiProfileDedupKey(profile)) &&
        !hasEquivalentApiProfile(current.profiles, profile),
    )
    .map((profile) => ({
      ...profile,
